...
run-cluster: ## run mysql cluster
	docker run --rm -v $(shell pwd)/my.cnf:/etc/mysql/conf.d/my.cnf -d -e MYSQL_ROOT_PASSWORD=root -e CLUSTER_NAME=cluster_osoc -p 3307:3306 --name=osoc_node1 --net=osoc_default percona/percona-xtradb-cluster:5.7
	sleep 30
	...
	sleep 5
	docker run --rm -v $(shell pwd)/my.cnf:/etc/mysql/conf.d/my.cnf -d -e MYSQL_ROOT_PASSWORD=root -e CLUSTER_NAME=cluster_osoc -e CLUSTER_JOIN=osoc_node1 -p 3309:3306 --name=osoc_node3 --net=osoc_default percona/percona-xtradb-cluster:5.7
